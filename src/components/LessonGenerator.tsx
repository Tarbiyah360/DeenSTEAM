...
const LessonGenerator = () => {
  ...
  const ageToYearMap: Record<string, {
    year: number;
    yearLabel: string;
  }> = {
    "5": {
  ...
  const curriculumTopics = ["Forces and Magnets", "Light and Shadows", "Electricity", "Sound", "States of Matter", "Living Things and Their Habitats", "Plants", "Animals Including Humans", "Rocks and Soils", "Earth and Space", "Properties of Materials", "Evolution and Inheritance"];
  const generateLesson = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke("generate-lesson", {
        body: {
          age: parseInt(age),
          year: yearData.year,
          topic
  ...
  return <section id="lesson-generator" className="py-20 px-4" aria-labelledby="lesson-heading">
      <div className="container mx-auto max-w-5xl">
        ...
        <Card className="p-6 mb-8 bg-card/50 border-border/50">
          <h3 className="text-xl font-semibold text-foreground mb-4 text-center">How It Works</h3>
          <div className="grid md:grid-cols-3 gap-6">
            <div className="text-center space-y-2">
              <div className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary text-primary-foreground font-bold mb-2">1</div>
              <h4 className="font-semibold text-foreground">Select Age</h4>
              <p className="text-sm text-muted-foreground">Choose your child's age (5-11 years)</p>
            </div>
            <div className="text-center space-y-2">
              <div className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary text-primary-foreground font-bold mb-2">2</div>
              <h4 className="font-semibold text-foreground">Pick a Topic</h4>
              <p className="text-sm text-muted-foreground">Select from UK STEM curriculum topics</p>
            </div>
            <div className="text-center space-y-2">
              <div className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary text-primary-foreground font-bold mb-2">3</div>
              <h4 className="font-semibold text-foreground">Generate Lesson</h4>
              <p className="text-sm text-muted-foreground">Get an Islamic-inspired STEM lesson plan</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6 md:p-8 mb-8 shadow-lg border-border/50">
          ...
          e.preventDefault();
          generateLesson();
        }} className="space-y-6" role="search" aria-label="Lesson generation form">
            <div className="space-y-2">
              ...
              {age && <p className="text-sm text-muted-foreground mt-2">
            ...
            <div className="space-y-2">
              ...
              <Select value={topic} onValueChange={setTopic}>
                ...
                <SelectContent>
                  {curriculumTopics.map(curriculumTopic => <SelectItem key={curriculumTopic} value={curriculumTopic}>
                      {curriculumTopic}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <Button type="submit" disabled={isLoading || !age || !topic} size="lg" className="w-full" aria-label={isLoading ? "Generating lesson plan" : "Generate lesson plan"}>
              {isLoading ? <>
